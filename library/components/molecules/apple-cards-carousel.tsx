  ...
  return (
    <>
      ...
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className={`rounded-3xl ${card.bg}  h-80 w-56 md:h-[40rem] md:w-96 overflow-hidden flex flex-col items-start justify-start relative z-10`}
      >
        <div className="absolute h-full top-0 inset-x-0 bg-gradient-to-b from-black/20 via-transparent to-transparent z-30 pointer-events-none" />
        <div className="relative z-40 p-8">
          ...
          <motion.p
            layoutId={layout ? `title-${card.title}` : undefined}
            className="text-white text-xl md:text-3xl font-semibold max-w-xs text-left [text-wrap:balance] font-sans mt-2 leading-[1000px]"
          >
            {card.title}
          </motion.p>
        </div>
